...
/**
 * @version $Id: cms.php 290 2012-09-20 15:27:03Z cmb69 $
 */
...
define('CMSIMPLE_XH_VERSION', 'CMSimple_XH 1.5.4');
define('CMSIMPLE_XH_BUILD', '2012092201');
define('CMSIMPLE_XH_DATE', '2012-09-22');
...
// yen add this
$download_path = $pth['folder']['downloads'];
...
$a_browser_data = browser_detection('full');
	...
	$utf8_filename = str_replace($download_path,"",$fl);
	$big5_filename = iconv("utf-8","big-5",$utf8_filename);
    
if (substr(php_uname(), 0, 7) == "Windows")
{
	$filename = $big5_filename;
	$full_filename = $download_path.$big5_filename;
}
else
{
	$filename = $utf8_filename;
	$full_filename = $download_path.$utf8_filename;
}
	if (!is_readable($full_filename) || ($download != '' && !chkdl($sn.'?download='.$filename))) {
		global $o, $text_title;
		
		shead('404');
		
		$o .= '<p>File '.$utf8_filename.'</p>';
		return;
	} else {
...
else
{
	if ( $a_browser_data[1] >= 5 )
	{
       ...
       {
	       	$big5_filename=stripslashes($big5_filename);
       }

	header('Content-Disposition: attachment; filename="'.$big5_filename.'"');
	header('Content-Length:'.filesize($full_filename));
	}
}
